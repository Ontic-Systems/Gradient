...
> Issues: [#357](https://github.com/Ontic-Systems/Gradient/issues/357) (lexer + parser) + [#358](https://github.com/Ontic-Systems/Gradient/issues/358) (checker + IR builder) — full closure of adversarial-review an adversarial-review finding.
> Epic: [#302](https://github.com/Ontic-Systems/Gradient/issues/302) (threat model).

The Gradient compiler ships a [cargo-fuzz](https://rust-fuzz.github.io/book/cargo-fuzz.html) harness with libFuzzer-driven coverage-guided fuzzing of every parser-tier through IR-tier entry point. Together with #357 and #358 this closes the the deliverable: "no panics in 24h soak required before public push."
...
1. **Nightly soak** — cron `0 2 * * *`. Two parallel jobs (one per target), each running 4 hours (14400s). Per-job timeout 290 minutes (under the 6-hour GHA hard cap with buffer). Per the related finding acceptance: "Zero panics in 24hr soak required before public push" — six consecutive nightly greens cover that.
