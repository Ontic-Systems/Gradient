...
> **STATUS:** partial — Grammar-constrained decoding, runtime contracts, effects, and the Query API are implemented. Static `@verified` SMT discharge ships as **opt-in via `GRADIENT_VC_VERIFY=1`** with one-hundred-seventy-eight stdlib functions across eighteen modules (`core_math.gr`, `core_bool.gr`, `core_compare.gr`, `core_int_ops.gr`, `core_arith_ops.gr`, `core_order_ops.gr`, `core_pair_ops.gr`, `core_select_ops.gr`, `core_chain_ops.gr`, `core_witness_ops.gr`, `core_parity_ops.gr`, `core_interval_ops.gr`, `core_neg_ops.gr`, `core_inequality_chain_ops.gr`, `core_assoc_ops.gr`, `core_swap_ops.gr`, `core_zero_one_ops.gr`, `core_successor_ops.gr`) discharged on every CI run (see [ADR 0003 step 5 / sub-issue #331](https://github.com/Ontic-Systems/Gradient/issues/331)); broader stdlib coverage, `@untrusted` source mode, and capability-scoped manifests remain planned (Epics #297, #302, #303).
...
The stdlib `@verified` corpus now spans eighteen modules:
...
- [`compiler/stdlib/core_assoc_ops.gr`](../codebase/compiler/stdlib/core_assoc_ops.gr) — associativity / re-grouping slice. Ten functions (`add4_left`, `add4_right`, `add4_pair`, `add5_left`, `add5_pair_right`, `rotate_left3`, `rotate_right3`, `sub_left_assoc`, `sub_via_add_neg`, `add2_sub2_regroup`) covering 4- and 5-term left/right/paired additive re-groupings, 3-term commutative rotation, subtraction associativity, sub-as-add-neg conversion, and combined add/sub regroupings — all stress-testing wider re-groupings inside linear-integer arithmetic (10 obligations).
- [`compiler/stdlib/core_swap_ops.gr`](../codebase/compiler/stdlib/core_swap_ops.gr) — swap-symmetry / argument-permutation slice. Ten functions (`add_swap`, `sub_swap_negated`, `add3_swap_first_two`, `add3_swap_last_two`, `add3_full_reverse`, `sum_with_negated_self`, `add_swap_dual_witness`, `diff_when_equal`, `mirror_difference`, `cancel_via_swap`) covering 2- and 3-argument additive swap symmetry, subtraction swap as negation, full reversal, additive-inverse self-cancellation, dual-witness swap with two postconditions, equal-input difference, and constant-zero swap-cancellation identities (11 obligations).
- [`compiler/stdlib/core_zero_one_ops.gr`](../codebase/compiler/stdlib/core_zero_one_ops.gr) — zero/one-identity slice. Ten functions (`add_zero_right`, `add_zero_left`, `sub_zero_right`, `self_difference`, `zero_plus_zero`, `add_when_first_zero`, `add_when_second_zero`, `add_zero_dual_witness`, `negate_zero`, `redundant_zero_chain`) covering additive identity (`n + 0 == n`, `0 + n == n`), subtractive identity (`n - 0 == n`), self-difference (`n - n == 0`), zero-summation, pre-conditioned zero-input shortcuts, dual-witness zero identities, zero negation, and redundant zero chains (11 obligations).
- [`compiler/stdlib/core_successor_ops.gr`](../codebase/compiler/stdlib/core_successor_ops.gr) — successor/predecessor slice. Twelve functions (`succ`, `pred`, `succ_then_pred`, `pred_then_succ`, `succ_succ`, `pred_pred`, `succ_diff_self`, `self_diff_pred`, `succ_strictly_greater`, `pred_strictly_less`, `succ_pred_difference`, `succ_dual_witness`) covering successor / predecessor identities (`n + 1` / `n - 1`), cancellation under composition, shift constants (`n + 2`, `n - 2`), unit difference (`(n+1) - n == 1`, `n - (n-1) == 1`), strict ordering predicates over the successor, the two-step difference between successor and predecessor (`(n+1) - (n-1) == 2`), and dual-witness commutativity (`n + 1 == 1 + n`) (13 obligations).

Every function exercises the parser → AST → checker → VC encoder → Z3 path on every CI run. The dedicated `verified` lane in [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) installs Z3 and pins `GRADIENT_Z3_REQUIRED=1`, so a regression that breaks the discharge path turns the lane red rather than silently skipping. This anchors the "compiler-verified" claim to a continuously-checked artifact and tracks an adversarial-review item's request that the marketing language match a real verified subset (see [ADR 0003 step 5](adr/0003-tiered-contracts.md)).
