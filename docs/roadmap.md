...
| 4 | Tiered contracts (runtime + `@verified` SMT + `@runtime_only` opt-out) — see [ADR 0003](adr/0003-tiered-contracts.md) | [#297](https://github.com/Ontic-Systems/Gradient/issues/297) | in progress (ADR 0003 accepted; `@verified` annotation parses + checker recognizes #327; VC generator emits SMT-LIB for the supported subset #328; Z3 subprocess discharger + counterexample diagnostics #329 — opt-in via `GRADIENT_VC_VERIFY=1`; `@runtime_only(off_in_release)` opt-out + release audit #330; stdlib `@verified` pilot expanded to eleven modules — `core_math.gr` (10 fns / 17 obligations) + `core_bool.gr` (6 fns / 6 obligations) + `core_compare.gr` (10 fns / 11 obligations) + `core_int_ops.gr` (10 fns / 11 obligations) + `core_arith_ops.gr` (10 fns / 10 obligations) + `core_order_ops.gr` (10 fns / 14 obligations) + `core_pair_ops.gr` (10 fns / 17 obligations) + `core_select_ops.gr` (10 fns / 11 obligations) + `core_chain_ops.gr` (10 fns / 10 obligations) + `core_witness_ops.gr` (10 fns / 15 obligations) + `core_parity_ops.gr` (10 fns / 17 obligations) — discharged on every CI run #331) |
