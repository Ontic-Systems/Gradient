...
| 2 | Effect-tier foundation (`!{Heap}`/`!{Stack}`/`!{Static}`/`!{Async}`/`!{Atomic}`/`!{Volatile}`/`!{Throws}`) + `@panic(abort\|unwind\|none)` module attribute — see [ADR 0001](adr/0001-effect-tier-foundation.md) | [#295](https://github.com/Ontic-Systems/Gradient/issues/295) | in progress (ADR 0001 accepted; `!{Heap}` gates heap allocation; `!{Stack}`/`!{Static}` are known marker effects; `@panic(abort\|unwind\|none)` parses + checker rejects integer division/modulo under `@panic(none)` #318; **string concatenation `+` propagates `Heap` to caller [#531](https://github.com/Ontic-Systems/Gradient/issues/531) — closes audit-trail leak made user-visible by `@no_std` #348**) |
| 3 | Capability + arena memory model (typestate caps, arenas, C ABI, `Unsafe` gate on `extern`) — see [ADR 0002](adr/0002-arenas-capabilities.md) | [#296](https://github.com/Ontic-Systems/Gradient/issues/296) | in progress (ADR 0002 accepted; `FFI(C)` audit-trail effect on every `extern fn` landed [#322](https://github.com/Ontic-Systems/Gradient/issues/322) launch tier; runtime-crate half of [#320](https://github.com/Ontic-Systems/Gradient/issues/320) landed via [#543](https://github.com/Ontic-Systems/Gradient/pull/543) — `@allocator(arena)` runtime variant; **language-side `!{Arena(<name>)}` parameterized effect landed via this PR — parser + checker + Query API surface; binding/lifetime/typestate enforcement deferred to [#321](https://github.com/Ontic-Systems/Gradient/issues/321)**) |
| 4 | Tiered contracts (runtime + `@verified` SMT + `@runtime_only` opt-out) — see [ADR 0003](adr/0003-tiered-contracts.md) | [#297](https://github.com/Ontic-Systems/Gradient/issues/297) | in progress (ADR 0003 accepted; `@verified` annotation parses + checker recognizes #327; VC generator emits SMT-LIB for the supported subset #328; Z3 subprocess discharger + counterexample diagnostics #329 — opt-in via `GRADIENT_VC_VERIFY=1`; `@runtime_only(off_in_release)` opt-out + release audit #330; stdlib `@verified` pilot expanded to eighteen modules — `core_math.gr` (10 fns / 17 obligations) + `core_bool.gr` (6 fns / 6 obligations) + `core_compare.gr` (10 fns / 11 obligations) + `core_int_ops.gr` (10 fns / 11 obligations) + `core_arith_ops.gr` (10 fns / 10 obligations) + `core_order_ops.gr` (10 fns / 14 obligations) + `core_pair_ops.gr` (10 fns / 17 obligations) + `core_select_ops.gr` (10 fns / 11 obligations) + `core_chain_ops.gr` (10 fns / 10 obligations) + `core_witness_ops.gr` (10 fns / 15 obligations) + `core_parity_ops.gr` (10 fns / 17 obligations) + `core_interval_ops.gr` (10 fns / 21 obligations) + `core_neg_ops.gr` (10 fns / 10 obligations) + `core_inequality_chain_ops.gr` (10 fns / 10 obligations) + `core_assoc_ops.gr` (10 fns / 10 obligations) + `core_swap_ops.gr` (10 fns / 11 obligations) + `core_zero_one_ops.gr` (10 fns / 11 obligations) + `core_successor_ops.gr` (12 fns / 13 obligations) — discharged on every CI run #331) |
| 5 | Modular runtime (refcount/actors/async/allocator/panic as effect-driven linkable units) | [#298](https://github.com/Ontic-Systems/Gradient/issues/298) | in progress (panic strategy abstraction landed [#337](https://github.com/Ontic-Systems/Gradient/issues/337) — three runtime crates `runtime_panic_{abort,unwind,none}.c` selected at link time by `Module.panic_strategy`; `gradient build` reads `@panic(...)` from main and links the matching object; embedded `@panic(none)` fixture builds without `__gradient_panic` because the checker rejected every panic-able op at compile time. **Alloc-strategy split landed** [#333](https://github.com/Ontic-Systems/Gradient/issues/333) — two runtime crates `runtime_alloc_{full,minimal}.c` selected automatically by effect surface (`full` if `Heap` reachable, `minimal` otherwise — driven by ADR 0005's effect-driven DCE commitment, NOT a user-facing attribute); `gradient build` reads `effects_used` and links the matching object; binary-size delta reported in verbose mode; embedded `tests/embedded_no_heap.gr` fixture proves heap-free programs link the minimal variant; rc/COW machinery extraction from `gradient_runtime.c` deferred to a follow-on PR — today the variant runtimes are tag-only, the dispatch is wired). **Actor-strategy split landed** [#334](https://github.com/Ontic-Systems/Gradient/issues/334) — two runtime crates `runtime_actor_{full,none}.c` selected automatically by effect surface (`full` if `Actor` reachable, `none` otherwise); `gradient build` derives the strategy from `effects_used` and links the matching object; embedded `tests/embedded_no_actor.gr` fixture proves actor-free programs link the `none` variant on every CI run; actor-scheduler extraction from `gradient_runtime.c` parked for a follow-on PR with measurable size delta. **Async-strategy split landed** [#335](https://github.com/Ontic-Systems/Gradient/issues/335) — two runtime crates `runtime_async_{full,none}.c` selected automatically by effect surface (`full` if `Async` reachable, `none` otherwise); sibling of #334 with `Async` as the trigger effect; embedded `tests/embedded_no_async.gr` fixture proves async-free programs link the `none` variant; verbose binary-size line now reads `Binary size: N bytes (alloc=<>, panic=<>, actor=<>, async=<>)`; async-executor extraction from `gradient_runtime.c` parked for a follow-on PR with measurable size delta. **Hello-world size-budget CI gate landed** [#338](https://github.com/Ontic-Systems/Gradient/issues/338) — new `size-budget` CI lane builds two representative `gradient build` outputs (embedded pure-arith + IO; full app-mode with `Heap` via String concat), strips them, and asserts post-strip size stays under absolute byte budgets so future PRs that bloat the canonical runtime or any modular variant runtime get caught at CI time; budgets set generously today (variant runtimes are tag-only; canonical runtime carries libcurl) and tighten as the rc/COW + actor-scheduler + async-executor extractions land. **Allocator-strategy split landed** [#336](https://github.com/Ontic-Systems/Gradient/issues/336) — two runtime crates `runtime_allocator_{default,pluggable}.c` selected at link time by `Module.allocator_strategy`; `default` (the AST default) wraps libc `malloc`/`free` behind `__gradient_alloc`/`__gradient_free`, while `pluggable` declares those symbols `extern` so embedders can supply a bumpalo arena, slab, or other custom allocator under the same C ABI vtable; `gradient build` reads `@allocator(default | pluggable)` from main and links the matching object; verbose binary-size line extended to `Binary size: N bytes (alloc=<>, panic=<>, actor=<>, async=<>, allocator=<>)`; embedded `tests/embedded_allocator_pluggable.gr` fixture proves heap-free pluggable programs link cleanly without an embedder vtable because the alloc/free symbol references DCE out; bumpalo + slab implementations of the `pluggable` variant deferred to a follow-on PR — today the `pluggable` variant supplies the trait surface only, the integration test covers contract round-trip. **Arena allocator runtime crate landed** ([#320](https://github.com/Ontic-Systems/Gradient/issues/320) follow-on) — third allocator variant `runtime_allocator_arena.c` selected by `@allocator(arena)`; supplies its OWN `__gradient_alloc` / `__gradient_free` bodies backed by a process-global bump-pointer arena (vendored from `runtime/memory/arena.c`, GRA-178 hardened); `__gradient_free` is a no-op — bulk reclamation happens at process exit via an `atexit` hook; first concrete `pluggable`-class implementation under the same C ABI vtable; embedded `tests/embedded_allocator_arena.gr` fixture proves the attribute round-trips cleanly through the e2e CI lane) |
| 6 | Backend split (Cranelift dev / LLVM release + cross-compile + DWARF) — see [ADR 0004](adr/0004-cranelift-llvm-split.md) | [#299](https://github.com/Ontic-Systems/Gradient/issues/299) | in progress (LLVM backend buildable + scaffold-tested via [#339](https://github.com/Ontic-Systems/Gradient/issues/339); print/numeric/`pow`/string/float/time builtin lowerings; cross-compile via `--target <triple>` plumbed through to LLVM `TargetMachine` via [#342](https://github.com/Ontic-Systems/Gradient/issues/342) launch tier — RISC-V + ARM verified end-to-end, nightly cross-compile CI matrix follow-up planned) |
...
| 8 | Inference engine + `@app`/`@system` modes — see [ADR 0006](adr/0006-inference-modes.md) | [#301](https://github.com/Ontic-Systems/Gradient/issues/301) | started (`@app`/`@system` module mode parses + checker `@system` enforcement on `FnDef` return type and effect set per [#352](https://github.com/Ontic-Systems/Gradient/issues/352); Query API surfaces `module_mode`. Bidirectional inference engine pending [#350](https://github.com/Ontic-Systems/Gradient/issues/350)/[#351](https://github.com/Ontic-Systems/Gradient/issues/351)/[#353](https://github.com/Ontic-Systems/Gradient/issues/353).) |
...
| 11 | Tooling suite (bench/doc/asm/cross-compile/bindgen/DWARF + plugin spec) | [#304](https://github.com/Ontic-Systems/Gradient/issues/304) | in progress (`gradient bench` MVP landed via [#371](https://github.com/Ontic-Systems/Gradient/issues/371) — `@bench` annotation parses + checker validates; `gradient bench [--filter] [--baseline] [--json]` discovers `@bench` fns, runs each in an auto-tuned timed loop, emits stable JSON `{schema_version: 1, benches: [...]}`; baseline-compare flags >10% regression with non-zero exit. `gradient doc --html` full surface landed via [#372](https://github.com/Ontic-Systems/Gradient/issues/372) — renders self-contained static HTML site to `target/doc/` with per-fn effect badges, capability ceiling display, contracts/budget rendering, doc comments, and a vanilla-JS client-side search box; `search-index.json` schema pinned at version 1. **Plugin protocol v1 landed via [#377](https://github.com/Ontic-Systems/Gradient/issues/377) — `gradient <name>` dispatches to a `gradient-<name>` binary on PATH; protocol pinned at version 1 with handshake env vars (`GRADIENT_PLUGIN_PROTOCOL_VERSION`, `GRADIENT_VERSION`, `GRADIENT_BIN`, `GRADIENT_PROJECT_ROOT`); built-in subcommands always shadow same-named plugins; reference plugin `gradient-hello-plugin` ships under `docs/plugins/`; full spec at `docs/plugins/protocol.md`.**) |
