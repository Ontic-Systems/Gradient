...
| 4 | Tiered contracts (runtime + `@verified` SMT + `@runtime_only` opt-out) — see [ADR 0003](adr/0003-tiered-contracts.md) | [#297](https://github.com/Ontic-Systems/Gradient/issues/297) | in progress (ADR 0003 accepted; `@verified` annotation parses + checker recognizes #327; VC generator emits SMT-LIB for the supported subset #328; Z3 subprocess discharger + counterexample diagnostics #329 — opt-in via `GRADIENT_VC_VERIFY=1`; `@runtime_only(off_in_release)` opt-out + release audit #330; stdlib `@verified` pilot expanded to two modules — `core_math.gr` (10 fns / 17 obligations) + `core_bool.gr` (6 fns / 6 obligations) — discharged on every CI run #331) |
