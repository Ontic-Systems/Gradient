...
//! over the k-step inequality chain slice (sibling to the existing
//! pilot tests).
//!
//! Loads `compiler/stdlib/core_inequality_chain_ops.gr` — the
//! fourteenth stdlib module shipped under `@verified` — and runs every
//! `@verified fn` declared there through the [`ContractDischarger`].
//! Asserts every contract obligation comes back `Discharged`,
//! end-to-end:
//!
...
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_inequality_chain_ops.gr")
}
...
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!(
        "[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the inequality-chain-ops stdlib pilot"
    );
    None
}
...
fn load_verified_fns() -> Vec<FnDef> {
    let src =
        std::fs::read_to_string(pilot_path()).expect("read inequality-chain-ops stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(
        errs.is_empty(),
        "inequality-chain-ops stdlib pilot must parse cleanly: {errs:?}"
    );
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "inequality-chain-ops stdlib pilot must declare at least one @verified fn"
    );
    out
}

#[test]
fn stdlib_inequality_chain_ops_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "inequality-chain-ops stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
    ...
    assert!(
        ...
        "expected at least 10 discharged obligations across the inequality-chain-ops pilot, got {total_obligations}"
    );
}

#[test]
fn stdlib_inequality_chain_ops_pilot_named_landmark_fns_present() {
    ...
    for required in [
        ...
        "ge_chain_four",
        "pos_chain_sum_four",
        "nonneg_chain_sum_five",
    ] {
        assert!(
            names.iter().any(|n| n == required),
            "inequality-chain-ops stdlib pilot must declare `{required}`; found {names:?}"
        );
    }
}
