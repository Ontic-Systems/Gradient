...
//! over the Bool-algebra slice (sibling to `vc_stdlib_pilot.rs`).
//!
//! Loads `compiler/stdlib/core_bool.gr` — the second stdlib module
...
//!
//! Like `tests/vc_stdlib_pilot.rs`, this test skips cleanly when Z3
//! is unavailable so CI lanes that don't install Z3 stay green. The
...
//! green build.
//!
//! Adding a new `@verified fn` to `compiler/stdlib/core_bool.gr`
...
/// Path to the Bool-algebra stdlib pilot module. Resolved relative to
/// the `gradient-compiler` crate's `CARGO_MANIFEST_DIR` so the test
/// runs regardless of where the workspace was invoked from.
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_bool.gr")
}

/// Skip the test cleanly when Z3 isn't on PATH and the user hasn't
/// pinned `GRADIENT_Z3_REQUIRED=1`. Mirrors the pattern in
/// `tests/vc_z3_integration.rs` and `tests/vc_stdlib_pilot.rs`.
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!(
        "[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the bool stdlib pilot"
    );
    None
}
...
fn load_verified_fns() -> Vec<FnDef> {
    let src = std::fs::read_to_string(pilot_path()).expect("read bool stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(
        errs.is_empty(),
        "bool stdlib pilot must parse cleanly: {errs:?}"
    );
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "bool stdlib pilot must declare at least one @verified fn"
    );
    out
}
...
#[test]
fn stdlib_bool_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "bool stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
    ...
    // Sanity floor: the file declares 6 fns with at least one
    // obligation each (6 obligations total at the time of writing).
    // If a future edit drops below this, treat it as a regression of
    // the pilot's coverage rather than a passing test.
    assert!(
        total_obligations >= 6,
        "expected at least 6 discharged obligations across the bool pilot, got {total_obligations}"
    );
}

#[test]
fn stdlib_bool_pilot_named_landmark_fns_present() {
    ...
    for required in [
        ...
        assert!(
            ...
            "bool stdlib pilot must declare `{required}`; found {names:?}"
        );
    }
}
