...
//! over the comparison-reflection slice (sibling to `vc_stdlib_pilot.rs`
//! and `vc_stdlib_pilot_bool.rs`).
//!
//! Loads `compiler/stdlib/core_compare.gr` — the third stdlib module
...
//! green build.
//!
//! Adding a new `@verified fn` to `compiler/stdlib/core_compare.gr`
...
/// Path to the comparison-reflection stdlib pilot module. Resolved
/// relative to the `gradient-compiler` crate's `CARGO_MANIFEST_DIR` so
/// the test runs regardless of where the workspace was invoked from.
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_compare.gr")
}
...
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!(
        "[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the compare stdlib pilot"
    );
    None
}
...
fn load_verified_fns() -> Vec<FnDef> {
    let src = std::fs::read_to_string(pilot_path()).expect("read compare stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(
        errs.is_empty(),
        "compare stdlib pilot must parse cleanly: {errs:?}"
    );
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "compare stdlib pilot must declare at least one @verified fn"
    );
    out
}
...
#[test]
fn stdlib_compare_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "compare stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
    ...
    // sign_int contributes 2). If a future edit drops below this,
    // treat it as a regression of the pilot's coverage rather than a
    // passing test.
    assert!(
        total_obligations >= 11,
        "expected at least 11 discharged obligations across the compare pilot, got {total_obligations}"
    );
}

#[test]
fn stdlib_compare_pilot_named_landmark_fns_present() {
    ...
    for required in [
        ...
        assert!(
            ...
            "compare stdlib pilot must declare `{required}`; found {names:?}"
        );
    }
}
