...
//! over the integer-ordering slice (sibling to `vc_stdlib_pilot.rs`,
//! `vc_stdlib_pilot_bool.rs`, `vc_stdlib_pilot_compare.rs`,
//! `vc_stdlib_pilot_int_ops.rs`, and `vc_stdlib_pilot_arith_ops.rs`).
//!
//! Loads `compiler/stdlib/core_order_ops.gr` — the sixth stdlib module
...
//! green build.
//!
//! Adding a new `@verified fn` to `compiler/stdlib/core_order_ops.gr`
...
/// Path to the integer-ordering stdlib pilot module.
...
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_order_ops.gr")
}
...
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!(
        "[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the order-ops stdlib pilot"
    );
    None
}
...
fn load_verified_fns() -> Vec<FnDef> {
    let src = std::fs::read_to_string(pilot_path()).expect("read order-ops stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(
        errs.is_empty(),
        "order-ops stdlib pilot must parse cleanly: {errs:?}"
    );
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "order-ops stdlib pilot must declare at least one @verified fn"
    );
    out
}
...
#[test]
fn stdlib_order_ops_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "order-ops stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
    ...
    // Sanity floor: the file declares 10 fns with 14 obligations total
    ...
    assert!(
        ...
        "expected at least 14 discharged obligations across the order-ops pilot, got {total_obligations}"
    );
}

#[test]
fn stdlib_order_ops_pilot_named_landmark_fns_present() {
    ...
    for required in [
        ...
        assert!(
            ...
            "order-ops stdlib pilot must declare `{required}`; found {names:?}"
        );
    }
}
