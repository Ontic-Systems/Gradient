...
//! over the swap-symmetry / argument-permutation slice (sibling to
//! the existing pilot tests).
//!
//! Loads `compiler/stdlib/core_swap_ops.gr` — the sixteenth stdlib
...
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_swap_ops.gr")
}
...
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!(
        "[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the swap-ops stdlib pilot"
    );
    None
}

fn load_verified_fns() -> Vec<FnDef> {
    let src = std::fs::read_to_string(pilot_path()).expect("read swap-ops stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(
        errs.is_empty(),
        "swap-ops stdlib pilot must parse cleanly: {errs:?}"
    );
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "swap-ops stdlib pilot must declare at least one @verified fn"
    );
    out
}

#[test]
fn stdlib_swap_ops_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "swap-ops stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
    ...
    assert!(
        ...
        "expected at least 10 discharged obligations across the swap-ops pilot, got {total_obligations}"
    );
}

#[test]
fn stdlib_swap_ops_pilot_named_landmark_fns_present() {
    ...
    for required in [
        ...
        "add3_swap_last_two",
        "add3_full_reverse",
        "sum_with_negated_self",
        "add_swap_dual_witness",
        "diff_when_equal",
        "mirror_difference",
        "cancel_via_swap",
    ] {
        assert!(
            ...
            "swap-ops stdlib pilot must declare `{required}`; found {names:?}"
        );
    }
}
