...
//! over the interval / bound-preservation slice (sibling to `vc_stdlib_pilot.rs`,
...
//! `vc_stdlib_pilot_select_ops.rs`, `vc_stdlib_pilot_chain_ops.rs`,
//! `vc_stdlib_pilot_witness_ops.rs`, and `vc_stdlib_pilot_parity_ops.rs`).
//!
//! Loads `compiler/stdlib/core_interval_ops.gr` — the twelfth stdlib module
...
//! green build.
//!
//! Adding a new `@verified fn` to `compiler/stdlib/core_interval_ops.gr`
...
/// Path to the interval / bound-preservation stdlib pilot module.
...
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_interval_ops.gr")
}
...
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!(
        "[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the interval-ops stdlib pilot"
    );
    None
}
...
fn load_verified_fns() -> Vec<FnDef> {
    let src = std::fs::read_to_string(pilot_path()).expect("read interval-ops stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(
        errs.is_empty(),
        "interval-ops stdlib pilot must parse cleanly: {errs:?}"
    );
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "interval-ops stdlib pilot must declare at least one @verified fn"
    );
    out
}
...
#[test]
fn stdlib_interval_ops_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "interval-ops stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
    ...
    // Sanity floor: the file declares 10 fns with 21 obligations total
    ...
    assert!(
        ...
        "expected at least 21 discharged obligations across the interval-ops pilot, got {total_obligations}"
    );
}

#[test]
fn stdlib_interval_ops_pilot_named_landmark_fns_present() {
    ...
    for required in [
        ...
        "bump_above_lower",
        "drop_below_upper",
        "clamp_lower_one_sided",
        "clamp_upper_one_sided",
        "interval_width",
        "add_nonneg_preserves_lower",
        "reconstruct_upper_from_width",
    ] {
        assert!(
            names.iter().any(|n| n == required),
            "interval-ops stdlib pilot must declare `{required}`; found {names:?}"
        );
    }
}
