...
//! (sub-issue #331, ADR 0003 step 5).
//!
//! Loads `compiler/stdlib/core_math.gr` — the first stdlib module
...
//!
//! Like `tests/vc_z3_integration.rs`, this test skips cleanly when
//! Z3 is unavailable so CI lanes that don't install Z3 stay green.
//! The dedicated `verified` CI lane installs Z3 and pins
//! `GRADIENT_Z3_REQUIRED=1`, which makes a missing solver a hard
//! failure — so the discharge step is genuinely exercised on every
//! green build.
//!
//! Adding a new `@verified fn` to `compiler/stdlib/core_math.gr`
...
/// Path to the stdlib pilot module. Resolved relative to the
/// `gradient-compiler` crate's `CARGO_MANIFEST_DIR` so the test runs
/// regardless of where the workspace was invoked from.
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_math.gr")
}

/// Skip the test cleanly when Z3 isn't on PATH and the user hasn't
/// pinned `GRADIENT_Z3_REQUIRED=1`. Mirrors the pattern in
/// `tests/vc_z3_integration.rs`.
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!("[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the stdlib pilot");
    None
}
...
fn load_verified_fns() -> Vec<FnDef> {
    let src = std::fs::read_to_string(pilot_path()).expect("read stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(errs.is_empty(), "stdlib pilot must parse cleanly: {errs:?}");
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "stdlib pilot must declare at least one @verified fn"
    );
    out
}
...
#[test]
fn stdlib_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
...
#[test]
fn stdlib_pilot_named_landmark_fns_present() {
