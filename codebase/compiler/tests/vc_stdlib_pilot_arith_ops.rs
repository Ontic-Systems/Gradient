...
//! over the integer-arithmetic-axiom slice (sibling to
//! `vc_stdlib_pilot.rs`, `vc_stdlib_pilot_bool.rs`,
//! `vc_stdlib_pilot_compare.rs`, and `vc_stdlib_pilot_int_ops.rs`).
//!
//! Loads `compiler/stdlib/core_arith_ops.gr` — the fifth stdlib module
...
//! green build.
//!
//! Adding a new `@verified fn` to `compiler/stdlib/core_arith_ops.gr`
...
/// Path to the integer-arithmetic-axiom stdlib pilot module.
...
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_arith_ops.gr")
}
...
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!(
        "[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the arith-ops stdlib pilot"
    );
    None
}
...
fn load_verified_fns() -> Vec<FnDef> {
    let src = std::fs::read_to_string(pilot_path()).expect("read arith-ops stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(
        errs.is_empty(),
        "arith-ops stdlib pilot must parse cleanly: {errs:?}"
    );
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "arith-ops stdlib pilot must declare at least one @verified fn"
    );
    out
}
...
#[test]
fn stdlib_arith_ops_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "arith-ops stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
    ...
    // Sanity floor: the file declares 10 fns each with exactly one
    // obligation (10 obligations total at the time of writing). If a
    // future edit drops below this, treat it as a regression of the
    // pilot's coverage rather than a passing test.
    assert!(
        total_obligations >= 10,
        "expected at least 10 discharged obligations across the arith-ops pilot, got {total_obligations}"
    );
}

#[test]
fn stdlib_arith_ops_pilot_named_landmark_fns_present() {
    ...
    for required in [
        ...
        assert!(
            ...
            "arith-ops stdlib pilot must declare `{required}`; found {names:?}"
        );
    }
}
