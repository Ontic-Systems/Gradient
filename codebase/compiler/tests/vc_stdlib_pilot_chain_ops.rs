...
//! over the transitive-chain slice (sibling to `vc_stdlib_pilot.rs`,
...
//! and `vc_stdlib_pilot_select_ops.rs`).
//!
//! Loads `compiler/stdlib/core_chain_ops.gr` — the ninth stdlib module
...
//! green build.
//!
//! Adding a new `@verified fn` to `compiler/stdlib/core_chain_ops.gr`
...
/// Path to the transitive-chain stdlib pilot module.
...
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_chain_ops.gr")
}
...
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!(
        "[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the chain-ops stdlib pilot"
    );
    None
}
...
fn load_verified_fns() -> Vec<FnDef> {
    let src = std::fs::read_to_string(pilot_path()).expect("read chain-ops stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(
        errs.is_empty(),
        "chain-ops stdlib pilot must parse cleanly: {errs:?}"
    );
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "chain-ops stdlib pilot must declare at least one @verified fn"
    );
    out
}
...
#[test]
fn stdlib_chain_ops_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "chain-ops stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
    ...
    assert!(
        ...
        "expected at least 10 discharged obligations across the chain-ops pilot, got {total_obligations}"
    );
}

#[test]
fn stdlib_chain_ops_pilot_named_landmark_fns_present() {
    ...
    for required in [
        ...
        assert!(
            ...
            "chain-ops stdlib pilot must declare `{required}`; found {names:?}"
        );
    }
}
