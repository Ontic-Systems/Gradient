...
//! over the additive-negation slice (sibling to `vc_stdlib_pilot.rs`,
...
//! `vc_stdlib_pilot_witness_ops.rs`, `vc_stdlib_pilot_parity_ops.rs`,
//! and `vc_stdlib_pilot_interval_ops.rs`).
//!
//! Loads `compiler/stdlib/core_neg_ops.gr` — the thirteenth stdlib module
...
//! green build.
//!
//! Adding a new `@verified fn` to `compiler/stdlib/core_neg_ops.gr`
...
/// Path to the additive-negation stdlib pilot module.
...
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_neg_ops.gr")
}
...
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!(
        "[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the neg-ops stdlib pilot"
    );
    None
}
...
fn load_verified_fns() -> Vec<FnDef> {
    let src = std::fs::read_to_string(pilot_path()).expect("read neg-ops stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(
        errs.is_empty(),
        "neg-ops stdlib pilot must parse cleanly: {errs:?}"
    );
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "neg-ops stdlib pilot must declare at least one @verified fn"
    );
    out
}
...
#[test]
fn stdlib_neg_ops_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "neg-ops stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
    ...
    // Sanity floor: the file declares 10 fns with 10 obligations total
    ...
    assert!(
        ...
        "expected at least 10 discharged obligations across the neg-ops pilot, got {total_obligations}"
    );
}

#[test]
fn stdlib_neg_ops_pilot_named_landmark_fns_present() {
    ...
    for required in [
        "neg_via_zero_sub",
        "neg_cancels_input",
        "double_neg_returns_input",
        "neg_of_nonneg_is_nonpos",
        "neg_of_nonpos_is_nonneg",
        "sub_as_add_neg",
        "add_neg_self_is_zero",
        "neg_swap_diff",
        "neg_of_positive_is_negative",
        "neg_sum_distributes",
    ] {
        assert!(
            names.iter().any(|n| n == required),
            "neg-ops stdlib pilot must declare `{required}`; found {names:?}"
        );
    }
}
