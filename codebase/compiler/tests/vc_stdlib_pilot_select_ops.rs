...
//! over the conditional-selection slice (sibling to `vc_stdlib_pilot.rs`,
//! `vc_stdlib_pilot_bool.rs`, `vc_stdlib_pilot_compare.rs`,
//! `vc_stdlib_pilot_int_ops.rs`, `vc_stdlib_pilot_arith_ops.rs`,
//! `vc_stdlib_pilot_order_ops.rs`, and `vc_stdlib_pilot_pair_ops.rs`).
//!
//! Loads `compiler/stdlib/core_select_ops.gr` — the eighth stdlib module
...
//! green build.
//!
//! Adding a new `@verified fn` to `compiler/stdlib/core_select_ops.gr`
...
/// Path to the conditional-selection stdlib pilot module.
...
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_select_ops.gr")
}
...
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!(
        "[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the select-ops stdlib pilot"
    );
    None
}
...
fn load_verified_fns() -> Vec<FnDef> {
    let src = std::fs::read_to_string(pilot_path()).expect("read select-ops stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(
        errs.is_empty(),
        "select-ops stdlib pilot must parse cleanly: {errs:?}"
    );
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "select-ops stdlib pilot must declare at least one @verified fn"
    );
    out
}
...
#[test]
fn stdlib_select_ops_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "select-ops stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
    ...
    // Sanity floor: the file declares 10 fns with 11 obligations total
    ...
    assert!(
        ...
        "expected at least 11 discharged obligations across the select-ops pilot, got {total_obligations}"
    );
}

#[test]
fn stdlib_select_ops_pilot_named_landmark_fns_present() {
    ...
    for required in [
        ...
        assert!(
            ...
            "select-ops stdlib pilot must declare `{required}`; found {names:?}"
        );
    }
}
