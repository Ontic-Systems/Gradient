...
//! over the parity / multiple / step slice (sibling to `vc_stdlib_pilot.rs`,
...
//! `vc_stdlib_pilot_select_ops.rs`, `vc_stdlib_pilot_chain_ops.rs`, and
//! `vc_stdlib_pilot_witness_ops.rs`).
//!
//! Loads `compiler/stdlib/core_parity_ops.gr` — the eleventh stdlib module
...
//! green build.
//!
//! Adding a new `@verified fn` to `compiler/stdlib/core_parity_ops.gr`
...
/// Path to the parity / multiple / step stdlib pilot module.
...
fn pilot_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("stdlib/core_parity_ops.gr")
}
...
fn z3_or_skip() -> Option<ContractDischarger> {
    ...
    eprintln!(
        "[skip] Z3 not available; install z3 or set GRADIENT_Z3_BIN to run the parity-ops stdlib pilot"
    );
    None
}
...
fn load_verified_fns() -> Vec<FnDef> {
    let src = std::fs::read_to_string(pilot_path()).expect("read parity-ops stdlib pilot");
    let mut lexer = Lexer::new(&src, 0);
    let tokens = lexer.tokenize();
    let (module, errs) = parser::parse(tokens, 0);
    assert!(
        errs.is_empty(),
        "parity-ops stdlib pilot must parse cleanly: {errs:?}"
    );
    let mut out = Vec::new();
    ...
    assert!(
        !out.is_empty(),
        "parity-ops stdlib pilot must declare at least one @verified fn"
    );
    out
}
...
#[test]
fn stdlib_parity_ops_pilot_every_verified_fn_discharges() {
    ...
    for f in &fns {
        ...
        for q in &report.outcomes {
            match &q.outcome {
                ...
                other => panic!(
                    "parity-ops stdlib pilot `{}` obligation #{:?} did not discharge: {other:?}",
    ...
    assert!(
        ...
        "expected at least 17 discharged obligations across the parity-ops pilot, got {total_obligations}"
    );
}

#[test]
fn stdlib_parity_ops_pilot_named_landmark_fns_present() {
    ...
    for required in [
        ...
        assert!(
            ...
            "parity-ops stdlib pilot must declare `{required}`; found {names:?}"
        );
    }
}
