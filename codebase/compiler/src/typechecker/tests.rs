...
/// Assert that the source type-checks with no errors (warnings are allowed).
...
/// Assert that the source produces at least one type error (not warning)
...
/// Assert that the source produces exactly `n` type errors (not counting warnings).
...
fn match_bool_exhaustive_no_warning() {
    ...
    // non-exhaustiveness — the match is exhaustive.
    let src = "\
fn f(b: Bool) -> String:
    match b:
        true:
